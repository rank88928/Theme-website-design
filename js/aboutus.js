...
gsap.registerPlugin(ScrollTrigger);  //註冊插件
...
(function () {
    let banner_carousel_config = {
        ...
        txt: [
            document.querySelectorAll('.title-first'),
            document.querySelectorAll('.title-second'),
            document.querySelectorAll('.vertical-slogan')
        ],
    ...
    function txt_map(data) {
        ...
        for (let i = 0; i < data[0].length; i++) { //按第一標語數量劃分
            let sub_arr = []
...
// [config.dom.contact.people, config.dom.contact.form_box].forEach(function (item) {
//     fade.fromTo(
//         item,
//         {
//             opacity: 0,
...
//         })
// })
// config.dom.contact.form_group.forEach(function (item) {
...
//     fade.fromTo(
//         item, fade_params.start, fade_params.end)
// })
